...
solovoyStrassen n 0 = ssprimality n 
solovoyStrassen n k = ss `par` (ps `pseq` (result ss ps))
			where
			  	ss = ssprimality n
			  	ps = solovoyStrassen n (k-1)
...
millerRabin n 0 = millerRabinTest n 
millerRabin n k = mr `par` (pr `pseq` (result mr pr))
			     where
			     	  mr = millerRabinTest n  
			     	  pr = millerRabin n (k-1) 
...
millerRabinTest n  = do
			...
			setStdGen r2
			return (millerRabinPrimality n a)
